...
def create_limited_graph(min_nodes=5, max_nodes=20):
	...
	comuni = []
	with open('data/nomi.csv', 'r', encoding='utf-8') as file:
...
def readGraph ():
	...
	with open('results/predizione.txt', 'r') as f:
		righe = f.readlines()
		for riga in righe:
			line = riga[18:]
...
G_file = graph_file (graph=GPure, filename='results/input_pure.txt', original_to_prolog=original_to_prolog, prolog_to_original=prolog_to_original)
G_file = graph_file (graph=G, filename='results/input.txt', original_to_prolog=original_to_prolog, prolog_to_original=prolog_to_original)
