...
df = pd.read_csv ('code/comuni_completo.csv')
...
with open('code/cities.txt', 'r', encoding='utf-8') as f:
	content = f.read()
	cities = eval(content)
...
with open ('code/nomi.csv', 'w', encoding='utf-8') as f:
	for i in range(n):
		stringa = str(i) + ',' + nomi[i] + '\n'
		f.write (stringa)

with open ('code/dataset.csv', 'w', encoding='utf-8') as f:
	for i in range (n):
		for j in range (n):
