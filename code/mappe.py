...
class tsp_bb_agent:
	...
	def run_prolog(self):
		if self.filename == 'results/input_pure.txt':
...
agente_tsp_bb = tsp_bb_agent (graph=GPure, filename='results/input_pure.txt', out_file='results/tsp_results_pure.txt', original_to_prolog=original_to_prolog, prolog_to_original=prolog_to_original)

print('TSP pure graph:')
path = agente_tsp_bb.returnPath()

agente_tsp_bb = tsp_bb_agent (graph=G, filename='results/input.txt', out_file='results/tsp_results.txt', original_to_prolog=original_to_prolog, prolog_to_original=prolog_to_original)

print('TSP modified graph:')
...
for citta in [citta1,citta2]:
	...
	with open ('data/comuni.csv', 'r', encoding='utf-8') as f:
		reader = csv.reader(f)
		for row in reader:
	...
	if citta == citta1:
		mappa.save('results/city_route_map_pure.html')
	elif citta == citta2 or citta1 == citta2:
		mappa.save('results/city_route_map_with_weight.html')